...
if (
    ...
    $maxsql = "SELECT max(id) from lands";
    $maxresult = mysqli_query($connect, $maxsql);
    $maxcheck = mysqli_fetch_array($maxresult);
    $maxId = $maxcheck[0];
    $requiredId = $maxId + 1;

    $query = "INSERT INTO lands (id, land_title, location, land_desc, rent, area, owner_firstname, owner_lastname, phone_no, email) VALUES ('$requiredId','$land_title', '$location', '$land_desc', '$rent', '$area', '$owner_firstname', '$owner_lastname', '$phone_no', '$email')";
    if (mysqli_query($connect, $query)) {
        ...
        $data = ['success' => true, 'message' => ['Land added successfully']];
        echo json_encode($data);
    } else {
        $data = ['success' => false, 'message' => ['Land not added!']];
        echo json_encode($data);
    }
} else {
